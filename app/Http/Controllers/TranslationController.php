...
class TranslationController extends Controller
{
    ...
    public function deleteLanguageItem(Request $request)
    {
        ...
        $language_encrypt = Input::get('languageEncrypt');
    ...
    public function updateLanguage(Request $request)
    {
        ...
        $flag_name = $request->post('flag_name');
        $flag_code = $request->post('flag_code');
        $name = $request->post('name');
        $status = $request->post('status', 'off');
    ...
    public function getAllFlagCode()
    {
        if (empty(self::$_langs)) {
            $allFiles = $this->scanAllFiles(public_path('vendors/flag-icon-css/flags/4x3'));
            $codes = [];
            if (!empty($allFiles)) {
                foreach ($allFiles as $item) {
                    $code = substr($item, strlen($item) - 6, 2);
            ...
            if (!empty($files)) {
                foreach ($files as $file) {
                    if (file_exists($file)) {
                        ...
                        preg_match_all("/__\('(.*)'\)|__\('(.*)',|__\(\"(.*)\",|__\(\"(.*)\"\)|trans\('(.*)'\)|trans\(\"(.*)\"\)|trans_choice\('(.*)'\)|trans_choice\(\"(.*)\"\)|awe_lang\('(.*)'\)/U",
                            $content,
    ...
    {
        $translate = $request->post('trans', []);
        $lang = $request->post('lang', 'none');
        $langs = config('locales.languages');
    ...
    public function index(Request $request)
    {
        ...
        return view("dashboard.screens.{$folder}.translation", ['bodyClass' => 'hh-dashboard', 'strings' => $strings, 'trans' => $trans, 'langs' => $langs]);
    }
