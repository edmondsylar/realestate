...
class HomeAvailability extends Model
{
    ...
    public function getAvailabilityItems($home_id, $start, $end, $booking_id = null)
    {
        $start = esc_sql($start);
        $end = esc_sql($end);
        $sql = DB::table($this->getTable())->whereRaw("((start_date >= {$start} AND end_date <= {$end}) OR (start_date <= {$start} AND end_date >= {$start}) OR (start_date <= {$end} AND end_date >= {$end})) AND home_id = {$home_id}");
    ...
    public function getItem($home_id, $start_date, $end_date)
    {
