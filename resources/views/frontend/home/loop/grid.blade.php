<div class="hh-service-item grid" data-plugin="matchHeight">
    ...
    <div class="detail">
        <h2 class="title text-overflow"><a
                    href="{{ get_the_permalink($item->post_id, $item->post_slug) }}">{{ get_translate($item->post_title) }}</a></h2>
        ...
        <div class="price-wrapper {{ (empty($item->rating) || !enable_review()) ? 'left' : '' }}">
            <span class="price">{{ convert_price($item->base_price) }}</span><span class="unit">/{{get_home_booking_type($item)}}</span>
        </div>
    </div>
</div>
