                        ...
                        <div class="page-content-inner">
                            <div class="row">
                                @foreach($posts['results'] as $post)
                                    @php $is_blog_page = true @endphp
                                    <div class="col-md-6">
                                        <div class="post-item">
                                            <div class="post-thumbnail">
                                                <p class="post-date">{{ date(hh_date_format(), $post->created_at) }}</p>
                                                <a href="{{ get_home_permalink($post->post_id, $post->post_slug, 'post') }}">
                                                    @php
                                                        $thumb_id = $post->thumbnail_id;
                                                        $thumb_src = get_attachment_url($thumb_id, [550,270], true);
                                                    @endphp
                                                    <img src="{{ $thumb_src }}" alt="{{ get_attachment_alt($thumb_id) }}" class="img-fluid"/>
                                                </a>
                                            </div>
                                            <h3 class="post-title"><a href="{{ get_home_permalink($post->post_id, $post->post_slug, 'post') }}">
                                            ...
                                            <a href="{{ get_home_permalink($post->post_id, $post->post_slug, 'post') }}" class="readmore">
                                                {{__('Read more')}}
                                            </a>
                                        </div>
                                    </div>
                            ...
                            frontend_pagination([
                                ...
                                'posts_per_page' => posts_per_page('blog')
                            ]);
                            ?>
                        </div>
                    @else
