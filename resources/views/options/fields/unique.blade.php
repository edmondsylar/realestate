...
<div id="setting-{{ $idName }}" data-condition="{{ $condition }}"
    ...
    <input type="text" id="{{ $idName }}"
           ...
           class="form-control @if (isset($maxlengthHtml)) has-maxLength @endif @if (!empty($validation)) has-validation @endif" @if (isset($maxlengthHtml)) {{ $maxlengthHtml }} @endif
           name="{{ $id }}" value="{{ $value }}">
