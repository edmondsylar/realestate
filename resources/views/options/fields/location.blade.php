        ...
        <div class="col">
            <div class="form-group">
                ...
                @foreach($langs as $key => $item)
                    <input type="text" class="form-control hh-real-address @if (!empty($validation)) has-validation @endif {{get_lang_class($key, $item)}}" name="{{ $id }}[address]{{!empty($item) ? '['. $item .']' : ''}}"
                           id="{{ $idName }}_address{{get_lang_suffix($item)}}" value="{{ get_translate($value['address'], $item) }}" @if(!empty($item)) data-lang="{{$item}}" @endif>
                @endforeach
            </div>
        </div>
        ...
        <div class="col">
            <div class="mapbox-wrapper">
                <div class="form-group mapbox-text-search">
                    <div class="form-control" data-plugin="mapbox-geocoder" data-value="" data-placeholder="{{__('Search on map ...')}}"></div>
    ...
    <div class="row">
        <div class="col col-sm-6">
            <div class="form-group">
                ...
                @foreach($langs as $key => $item)
                    <input type="text" class="form-control hh-city {{get_lang_class($key, $item)}}" name="{{ $id }}[city]{{!empty($item) ? '['. $item .']' : ''}}"
                           id="{{ $idName }}_city{{get_lang_suffix($item)}}" value="{{ get_translate($value['city'], $item) }}" @if(!empty($item)) data-lang="{{$item}}" @endif>
                @endforeach
            </div>
        </div>
        <div class="col col-sm-6">
            <div class="form-group">
                ...
                @foreach($langs as $key => $item)
                    <input type="text" class="form-control hh-state {{get_lang_class($key, $item)}}" name="{{ $id }}[state]{{!empty($item) ? '['. $item .']' : ''}}"
                           id="{{ $idName }}_state{{get_lang_suffix($item)}}" value="{{ get_translate($value['state'], $item) }}" @if(!empty($item)) data-lang="{{$item}}" @endif>
                @endforeach
            </div>
        </div>
        <div class="col col-sm-6">
        ...
        <div class="col col-sm-6">
            <div class="form-group">
                ...
                @foreach($langs as $key => $item)
                    <input type="text" class="form-control hh-country {{get_lang_class($key, $item)}}" name="{{ $id }}[country]{{!empty($item) ? '['. $item .']' : ''}}"
                           id="{{ $idName }}_country{{get_lang_suffix($item)}}" value="{{ get_translate($value['country'], $item) }}" @if(!empty($item)) data-lang="{{$item}}" @endif>
                @endforeach
            </div>
        </div>
        <div class="col col-sm-6">
