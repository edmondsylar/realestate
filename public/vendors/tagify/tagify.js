...
Tagify.prototype = {
    ...
    templates : {
        ...
        tag(value, tagData){
            return `<tag title='${tagData.title || value}'
                ...
                <div>
                    <span class='tagify__tag-text'>${esc_html(value)}</span>
                </div>
            </tag>`
        },
    ...
    editTagDone(tagElm, tagData){
        ...
        if( tagData ){
            editableElm.textContent = esc_html(tagData.value)
            tagElm.title = esc_html(tagData.value)

            // update data
            tagElmIdx = this.getNodeIndex(tagElm)
            this.value[tagElmIdx].value = esc_html(tagData.value)
            this.update()
        }
    },
    ...
    input : {
        value : '',
        set( s = '', updateDOM = true, caretAtEnd ){
...
///////////////////////////)//////////////////////////
            ...
            if( this.settings.mode == 'select' ){
                ...
                this.input.set.call(this, tagData.value, true, true)
    ...
    createTagElem( tagData ){
        var tagElm,
            v = esc_html(this.escapeHTML(tagData.value)),
    ...
    getAttributes( data ){
        ...
        for( i=keys.length; i--; ){
            ...
            if( propName != 'class' && data.hasOwnProperty(propName) && data[propName] )
                s += " " + propName + (data[propName] ? `="${esc_html(data[propName])}"` : "");
        }
        return s;
    },
                    ...
                    switch( e.key ){
                        ...
                        case 'Enter' : {
                            ...
                            if( activeListElm ){
                                ...
                                if( this.settings.mode == 'select' )
                                    setTimeout(() => this.DOM.input.blur())
                                return false;
                            }
                            else{
